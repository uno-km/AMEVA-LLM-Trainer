"""
AMEVA LLM Trainer - Premium CLI Edition
메인 진입점 (cli/cli.py)
"""
import os
import sys
...
def print_header():
    ...
    console.print(Panel(
        ...
        "[dim]LLM Fine-tuning Engine · Headless Remote CLI · Powered by Rich[/dim]",
    ...
    with console.status("[yellow]서버 연결 확인 중...[/yellow]"):
        if not api_client.check_health():
            console.print(Panel(
                ...
                "[dim]명령어: uvicorn src.backend.main:app --host 0.0.0.0 --port 8001[/dim]",
                border_style="red"
            ))
            sys.exit(1)
