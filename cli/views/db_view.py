...
원격 DB 인스펙터 (API 기반) - tasks, metrics, logs 테이블 브라우저
"""
import os
import sys
...
def _run_query(sql: str, params=None) -> tuple:
    ...
    res = api_client.post("/api/v1/system/db/query", {"sql": sql, "params": params or []})
    if "error" in res:
...
def _run_custom_sql():
    console.clear()
    console.print(Panel(
        "[bold yellow]⚡ 커스텀 SELECT SQL 실행 (원격)[/bold yellow]",
        expand=False
    ))
    console.print("[dim]예시: SELECT * FROM tasks WHERE status='COMPLETED'[/dim]")
    console.print("[dim]      SELECT task_id, COUNT(*) FROM metrics GROUP BY task_id[/dim]\n")
