...
def _build_tree(parent: Tree, nodes: list):
    for node in sorted(nodes, key=lambda x: (not x.get('is_dir'), x.get('name', '').lower())):
        ...
        if node.get("is_dir"):
            ...
            icon = {
                "csv": "📊", "log": "📜", "db": "🗄️", "jsonl": "📋",
                "json": "📋", "yaml": "⚙️", "yml": "⚙️", "txt": "📝",
                "py": "🐍", "gguf": "🤖", "pt": "🧠", "bin": "🧠",
                "md": "📄", "docx": "📄"
            }.get(ext.lstrip('.'), "📄")
            parent.add(
                f"{icon} [white]{name}[/white]  [dim]{size}[/dim]  "
                f"[dim cyan]{node.get('path', '')}[/dim cyan]"
            )
...
def _view_file(path: str):
    console.clear()
    console.print(Panel(
        f"[bold]📝 원격 파일 뷰어: {os.path.basename(path)}[/bold]",
        expand=False
    ))

    with console.status("[yellow]서버에서 파일 내용을 불러오는 중...[/yellow]"):
        res = api_client.get("/api/v1/system/files/read", {"path": path})
    ...
    while True:
        ...
        console.print(Panel(
            ...
            f"[dim]총 {len(content)}줄/행  페이지 {page+1}/{total_pages}[/dim]",
            expand=False
        ))
        ...
        if content_type == "csv":
            ...
            for h in headers:
                table.add_column(str(h), style="cyan", max_width=30, overflow="fold")
            for row in rows:
        ...
        nav = Prompt.ask(
            "\n[dim]n=다음  p=이전  q=닫기[/dim]",
            choices=["n", "p", "q"], default="q"
        )
...
def _open_file_action(path: str):
    ...
    if ext in (".csv", ".log", ".txt", ".md", ".yaml", ".yml", ".json", ".jsonl"):
        _view_file(path)
    else:
...
def _search_files():
    ...
    with console.status(f"[yellow]서버에서 '{keyword}' 검색 중...[/yellow]"):
        res = api_client.get("/api/v1/system/files/search", {"keyword": keyword, "exts": exts})
...
def show_file_explorer():
    """파일 탐색기 메인"""
    CATEGORIES = {
        "1": ("dataset", "📦 Dataset (학습 데이터)"),
        "2": ("outputs", "🤖 Outputs (LoRA / 병합 모델 / 체크포인트)"),
        "3": ("logs", "📜 Logs"),
        "4": ("configs", "⚙️  Configs"),
    }
    ...
    while True:
        console.clear()
        console.print(Panel(
            "[bold cyan]📂 원격 파일 익스플로러 (API 기반)[/bold cyan]",
            expand=False
        ))
        ...
        with console.status("[yellow]서버에서 폴더 구조를 불러오는 중...[/yellow]"):
            res = api_client.get("/api/v1/system/files/explorer")
