...
LLM Loss 추이를 터미널에서 아스키 차트로 시각화
"""
import os
import sys
import time
...
def _build_plotext_chart(loss_hist, width=100, height=15):
    ...
    if loss_hist:
        plt.plot(loss_hist, marker="dot", color="red", label="Train Loss")
    else:
        plt.plot([0], [0], marker="dot")

    plt.title("실시간 학습 Loss 추이 (CPU LoRA Fine-tuning)")
    ...
    if not task_id:
        ...
        for i, t in enumerate(tasks, 1):
            status_color = {
                ...
                "FAILED": "red", "STOPPED": "magenta"
            }.get(t.get('status', ''), 'white')
            console.print(
                f"  [{i}] [{status_color}]{t.get('status', 'N/A'):10}[/{status_color}]  "
                f"{t.get('tsk_nm', 'N/A')}  [dim]({t.get('task_id', '')[:8]})[/dim]"
            )
        pick = IntPrompt.ask("\n번호", default=1)
        if pick < 1 or pick > len(tasks):
            return
        task_id = tasks[pick - 1].get('task_id', '')

    console.clear()
    console.print(f"[bold cyan]📡 원격 실시간 모니터 — Task {task_id[:8]} | Ctrl+C 종료[/bold cyan]\n")

    LEVEL_STYLE = {
        "INFO": "white", "ERROR": "bold red", "WARNING": "bold yellow",
    ...
    try:
        with Live(console=console, refresh_per_second=2, screen=False) as live:
            while True:
                ...
                log_table.add_column("시간", style="dim", width=20, no_wrap=True)
                ...
                status_bar.append(f"Progress: {progress:.1f}%", style="green")

                # Plotext 차트 렌더링
                terminal_width = console.measure("").maximum
                chart_width = min(terminal_width - 10, 100)
                chart_ansi = _build_plotext_chart(loss_history, width=chart_width, height=12)
                ...
                grid.add_row(Panel(
                    res_bar_txt,
                    title="[bold]🖥️ 서버 시스템 리소스[/bold]",
                    border_style="green", height=3
                ))
                grid.add_row(Panel(
                    log_table,
                    title=f"[bold]📜 실시간 로그 (최신 20개)[/bold]",
                    border_style="white"
                ))

                live.update(grid)

                if cur_status in ("COMPLETED", "FAILED", "STOPPED"):
                    time.sleep(2)
                    break
